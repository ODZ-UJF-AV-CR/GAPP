...
const services: FastifyPluginAsync = async (fastify) => {
    const telemetryService = new TelemetryService(fastify.telemetryRepository, fastify.vehiclesRepository, fastify.eventBus);
...
export default fp(services, { name: Plugins.SERVICES, dependencies: [Plugins.REPOSITORIES] });
